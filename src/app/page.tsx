...
export default function Home() {
  ...
  const loadCompany = useCallback(async (symbol: string) => {
    ...
    try {
      ...
      // Non-blocking: pull business summary + CEO from the Yahoo profile and merge in.
      fetch(`/api/company/${symbol}`, { cache: 'no-store' })
        .then(r => (r.ok ? r.json() : null))
        .then((p: any) => {
          if (!p) return;
          const extra: Partial<Company> = {};
          if (p.description) extra.description = p.description;
          if (p.ceo) extra.ceo = p.ceo;
          if (p.website) extra.website = p.website;
          if (p.employees) extra.employees = p.employees;
          if (Object.keys(extra).length === 0) return;
          setCompany(prev => (prev && prev.symbol === result.company.symbol ? { ...prev, ...extra } : prev));
          const c = _sessionCache.get(symbol); if (c) c.company = { ...c.company, ...extra };
        })
        .catch(() => {});
    } catch (err: any) {
            ...
            {isLoading ? (
              ...
              <div className="px-5 sm:px-8 lg:px-10 py-7 space-y-5 max-w-4xl mx-auto">
                ...
                {activeView === 'valuation' && financials.length > 0 && (
                  <>
                    {/* Quick context: what this company is */}
